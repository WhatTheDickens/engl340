...
Instead of typing `http://`  or `https://`, however, type `file://`, and follow this with the path to a location in your file hierarchy. For example, `file:///Users/` (Mac) or `file:///C:/Users/` (Windows) is the path to the `Users` folder that's standard in file systems. Typing the appropriate path to the `Users` folder on your machine, then hitting `enter` or `return` should display the contents of the folder in your browser. (*Note that there are 3 forward slashes (`///`) following `file:`. Typing `file://` tells your browser that you want to navigate your file hierarchy rather than the web. The third forward slash is actually part of the file path (`/Users/...`  or `/C:/Users/...`).*)
